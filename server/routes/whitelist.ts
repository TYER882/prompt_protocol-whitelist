...
const whitelistSchema = z.object({
  ...
  followedTwitter: z.literal(true, {
    errorMap: () => ({
      message: "Please complete the X follow task.",
    }),
  }),
});
...
router.post("/", submitLimiter, async (req, res) => {
  ...
  const { email, walletAddress, followedTwitter } =
    parsed.data;

  try {
    const existingEmail =
      await WhitelistEntry.findOne({ email });
    ...
    const existingWallet =
      await WhitelistEntry.findOne({ walletAddress });
    ...
    return res.status(500).json({
      success: false,
      message: "Whitelist server error.",
    });
  }
});
...
router.get("/count", async (_req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: "Failed to fetch whitelist count.",
    });
  }
});
